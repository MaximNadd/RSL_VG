...
def draw_bodypose_udp(canvas, candidate, subset, score):
    H, W, C = canvas.shape
    candidate = np.array(candidate)
    subset = np.array(subset)
    ...
    for i in range(len(limbSeq)):
        for n in range(len(subset)):
            ...
            kp1 = candidate[int(index[0])]
            kp2 = candidate[int(index[1])]
            if (kp1[0] == 0 and kp1[1] == 0) or (kp2[0] == 0 and kp2[1] == 0):
                continue  # Skip if any coordinate is zero
            Y = kp1[0] * float(W), kp2[0] * float(W)
            X = kp1[1] * float(H), kp2[1] * float(H)
            mX = np.mean(X)
            mY = np.mean(Y)
            length = ((X[0] - X[1]) ** 2 + (Y[0] - Y[1]) ** 2) ** 0.5
            angle = math.degrees(math.atan2(X[0] - X[1], Y[0] - Y[1]))
